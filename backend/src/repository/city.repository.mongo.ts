...
export class CityRepositoryMongo implements ICityRepository {
    async getAll(
        ...
        await connect()
        const query =
            citySearch && citySearch.name
                ? {
                      $or: [
                          { name: { $regex: citySearch.name, $options: 'i' } },
                          {
                              name_native: {
                                  $regex: citySearch.name,
                                  $options: 'i',
                              },
                          },
                      ],
                ...
                : {}
        const queryOpts = page
            ? { skip: (page.no - 1) * page.size, limit: page.size }
            : undefined
        const data: City[] = await CityModel.find(query, null, queryOpts).sort({
            name: 1,
        })
