...
describe("city service", () => {
    it("should get all cities", async () => {
        ...
        let result: PaginatedResponse<City> = await cityService.getAll();
    ...
    it("should get cities with page no 1 and size 4", async () => {
        let result: PaginatedResponse<City> = await cityService.getAll({}, { no: 1, size: 4 });
    ...
    it("should get cities with page no 2 and size 4", async () => {
        let result: PaginatedResponse<City> = await cityService.getAll({}, { no: 2, size: 4 });
    ...
    it("should get only Munich city data with search name 'Mun' page no 1 and size 4", async () => {
        let result: PaginatedResponse<City> = await cityService.getAll({ name: 'Mun' }, { no: 1, size: 4 });
