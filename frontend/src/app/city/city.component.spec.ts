...
describe('CityComponent', () => {
  ...
  describe("onSearchValueChanged", () => {
    ...
    it('should get cities when search value is changed to empty string and last searched value is not empty', () => {
      ...
      expect(cityServiceSpy.getAll).toHaveBeenCalledOnceWith({ name: '' }, { no: component.page.getValue(), size: component.pageSize });
    });
  });
  ...
  describe("search", () => {
    it("should get cities, set page to 1 and last searched value to form input's value when a search is called", () => {
      ...
      expect(cityServiceSpy.getAll).toHaveBeenCalledOnceWith({ name: searchValue }, { no: 1, size: component.pageSize });
    });
  });
  ...
  describe("selectPage", () => {
    it("should change page to given value and get cities when select page with number input is called", () => {
      ...
      expect(cityServiceSpy.getAll).toHaveBeenCalledOnceWith({ name: component.lastSearchedValue }, { no: newPage, size: component.pageSize });
    });
